...
export function render() {
    ...
    return `
        <div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-lg); padding-bottom: var(--spacing-sm); border-bottom: 1px solid #ddd;">
        ...
        <div class="top-nav-buttons" style="
            display: grid; 
            grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); /* 반응형 4분할 */
            gap: var(--spacing-md);
        ...
        <div class="dashboard-main-grid" style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--spacing-lg);">
            
            <div class="card user-activity-card">
                <h2 style="margin-bottom: var(--spacing-md);">User Activity</h2>
                <div id="activity-list" style="margin-top: var(--spacing-md); color: #6C757D; font-size: 0.95rem;">
                    <p>Loading activities...</p>
                </div>
            </div>

            <div class="summary-cards-container" style="display: flex; flex-direction: column; gap: var(--spacing-md);">
...
function makeCard(title, value, icon, color) {
    return `
        <div class="card summary-card" style="border-left: 5px solid ${color}; display: flex; justify-content: space-between; align-items: center; padding: var(--spacing-md) var(--spacing-lg); margin-bottom: 10px;">
            <div>
                <p style="font-size: 0.9rem; color: #6C757D; margin-bottom: 5px;">${title}</p>
                <h3 style="font-size: 1.8rem; color: ${color};">${value}</h3>
            </div>
...
function renderActivityList(activities) {
    ...
    `).join('') + `<p style="text-align: right; margin-top: 10px;"><a href="#" style="color: var(--color-primary);">View All &rarr;</a></p>`;
}
